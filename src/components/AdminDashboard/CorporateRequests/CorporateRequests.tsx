...
import CorporateRequestItem from './components/CorporateRequestItem'
...
export default function CorporateRequests() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() })) as CorporateRequest[]

      setRequests(data)
    } catch (err) {
      console.error('Error loading corporate requests', err)
      window.dispatchEvent(new CustomEvent('app:notify', { detail: { type: 'error', title: 'Error', message: 'Failed to load requests' } }))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <CorporateStats requests={requests} />
    </div>
  )
}
